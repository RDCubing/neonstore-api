    ...
    try {
        const response = await fetch(webhookUrl, {
            ...
            body: JSON.stringify({
                embeds: [
                    {
                        title: "New WebStore Application Submitted",
                        description: submission.Description,
                        author: {
                            name: submission.submittedUsername,
                            icon_url: submission.submittedAvatar || undefined
                        },
                        fields: [
                            {
                                name: "Application",
                            ...
                            {
                                name: "Framework / OS",
                                value: submission.Framework,
                                inline: true
                            },
                            {
                                name: "Submitted By",
                                value: submission.submittedUsername,
                                inline: true
                            },
                            {
                                name: "Status",
                        ...
                        thumbnail: {
                            url: submission.ImagePath
                        },
                        image: {
                            url: submission.DetailImagePath
                        },
                        footer: {
                            text: "Geek Devs Community • WebStore"
                        },
        ...
        console.error("Failed to send Discord submission embed:", err);
    }
}
...
router.post("/submit", async (req, res) => {
    try {
        ...
        if (!isValidUrl(ImagePath)) {
            return res.status(400).json({
                error: "Application icon URL is invalid."
            });
        }

        if (!isValidUrl(DetailImagePath)) {
            return res.status(400).json({
                error: "Screenshot URL is invalid."
            });
        }
        ...
        const submission = await AppSubmission.create({
            Title,
            Category,
            Subtitle,
            Publisher,
...
router.get("/mine", async (req, res) => {
    try {
        ...
        const submissions = await AppSubmission.find({
            submittedBy: user.id
        }).sort({
            createdAt: -1
        });
...
router.get("/stats", async (req, res) => {
    try {
        const approved = await AppSubmission.countDocuments({
            status: "approved"
        });

        const pending = await AppSubmission.countDocuments({
            status: "pending"
        });
